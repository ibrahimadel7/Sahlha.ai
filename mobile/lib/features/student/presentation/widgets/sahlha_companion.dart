...
/// with both back hands while speaking; only speech moves the mouth. Meaningful
...
class _CompanionPainter extends CustomPainter {
  ...
  /// Back-hand offsets from [GestureScheduler] in the painter's
  /// 100-unit space; zero rests each hand against the body.
  ...
  void paint(Canvas c, Size size) {
    ...
    // Two animated back hands resting against the body sides. While speaking
    // the scheduler eases them through small teaching gestures; at rest the
    // offsets decay to zero so both hands settle back onto the body.
    // Drawn before the body so the inner edges tuck behind it — never in
    // front of the face. There are no front hands.
    final handPaint = Paint()..color = const Color(0xFF0A7C76);
    c.drawCircle(Offset(9 + hands.leftDx, 68 + hands.leftDy), 8.5, handPaint);
    ...
    // No shapes are drawn in front of the body: only the two back hands
    // (drawn behind it above) animate. Nothing overlaps the body front.
